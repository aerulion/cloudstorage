...
public class OpenCloudExperienceTerminalListener implements Listener {
    @EventHandler
                    ...
                    if (Main.worldGuard.createProtectionQuery().testBlockBreak(event.getPlayer(), event.getClickedBlock())) {
                        ...
                        if (itemStack != null) {
                            if (itemStack.getType().equals(Material.WRITTEN_BOOK)) {
                                String uuid = NbtUtils.getNBTString(lectern.getInventory().getItem(0), NBT.KEY_CLOUD_EXPERIENCE_TERMINAL_OWNER_UUID.get());
                                if (!uuid.equals("")) {
                                    ...
                                    new FetchCloudExperienceTerminalTask(uuid, event.getPlayer(), Inventory.EXPERIENCE_TERMINAL);
                                }
                            }
                        }
                    }
